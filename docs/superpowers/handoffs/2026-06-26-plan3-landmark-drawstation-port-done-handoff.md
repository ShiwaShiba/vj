...
## 7. 追補（2026-06-26）— 西小屋 南斜面の半円ドーマー（眉窓）を移植

`addWingDormer` を新設し、`_drawStation` の「小屋南斜面に低く乗る半円ドーマー（eyebrow）」を移植：
- 小屋南斜面（`ST_REF` face 11 `[14,15,19,18]`）を skip→再構成。斜面は傾斜平面を `(u=plan-u, t=0南軒→1棟)` でパラメタ化。
- ドーマー＝`(u,t)` 空間の**上半楕円**（`half(t)=dr*sqrt(1-((t-dt0)/dt)^2)`＝下端 `dt0` で全幅・冠で点へ）。開口を斜面から抜き、背後に**盲穴ニッチ**（背面パネル＋内向きリム＝AOで暗化）。アーチ窓と同じ「窓＝凹み」言語で一貫。
- 斜面法線は `cross(û,t̂)` から解析（傾斜面の上＆南向き＝①カメラ向き）。リム法線は `quadToward`（ポケット中心へ向ける）。
- ノブ追加: `LM_DORMER`(on/off)`/U/R/T/T0/SEG`。既定 `du=-0.092, dr=0.020, dt=0.16, dt0=0.13, seg=8`（`_drawStation` 同値・env なしで再現）。
- test 追加（`assemble.test.mjs`）: 「小屋南斜面の半円ドーマーが凹む」（u帯×高さ帯で斜面前面 `zFront` より内側へ押し込まれた頂点数>15／実測38）。**citybake 36 / 全体 96 green**・決定論維持。
- RAYS=20 最終ベイク（seed=1 決定論）。CPU ラスタライザ orb0（南正面）で半円ドーマーが斜面の凹みとして読める・アーチ窓/背高窓/段差/canopy/周辺は不変。**manifest byte 不変**。
