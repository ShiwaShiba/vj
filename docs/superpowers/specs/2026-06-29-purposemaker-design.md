...
## 検証済みの見え方（look）
Hand_A を濃度サンプリング→白発光粒子 on 黒（8k/18k/34k点）で「黒地に光で描いた手」として美しく読めることを scratchpad ミックアップで確認済み（`shots`相当 `hand_mock.png`）。密度を上げるほど陰影が出る。**手は形＋陰影として読めるが、鉛筆の細かいハッチング線そのものは再現しない**（塵の濃淡として出る）。指は細いので低密度だと指先が淡くなる→既定 26k で安定。
...
## 粒子モデル（2.5D）
- 粒子状態: world `(x,y,z)`、`x,y∈[-1,1]`（正射影で screen 化）、`z∈[-1,1]`（乱流の奥行き）。FlowField 同様 prev 位置 `(px,py,pz)` も保持（streak 用）。
...
## Params / Modes
```
defineParam('count',    26000, 6000, 28000, 1000, 'Particles')
...
this.trail = 0.16
```
`count` 上限 28000 = bake 点数。`clock.quality` で実効 count を下方スケール（FlowField 同様）。
...
## Performance
26k 粒子＝FlowField 18k の depth-band stroke が iPad 実機で回る実績の範囲内。additive＋band 一括描画で stroke 数を抑制。`clock.quality` で動的減数。
