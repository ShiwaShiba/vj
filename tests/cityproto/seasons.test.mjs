...
import {
  ...
  AUTUMN_FRESH_COLOR, AUTUMN_MID_COLOR, agedAutumnColor,
...
test('秋の経年: 銀杏黄(age0)→オレンジ(中盤)→モミジ赤(age1=settled)、端点厳密一致・赤化は後半に圧縮', () => {
  assert.deepStrictEqual(agedAutumnColor(0, COLOR_PALETTE[2]), AUTUMN_FRESH_COLOR, 'age0=銀杏');
  assert.deepStrictEqual(agedAutumnColor(0.55, COLOR_PALETTE[2]), AUTUMN_MID_COLOR, 'AUTUMN_ORANGE_AT=オレンジ到達');
  assert.deepStrictEqual(agedAutumnColor(1, COLOR_PALETTE[2]), COLOR_PALETTE[2], 'age1=settled(赤)');
  // 緑(G)が単調に減って黄→橙→赤: 銀杏0.68 > オレンジ0.42 > 赤
  assert.ok(AUTUMN_FRESH_COLOR[1] > AUTUMN_MID_COLOR[1] && AUTUMN_MID_COLOR[1] > COLOR_PALETTE[2][1], '黄→橙→赤(G 単調減)');
  assert.notDeepStrictEqual(AUTUMN_MID_COLOR, COLOR_PALETTE[2], 'オレンジ ≠ 赤');
  // 赤化は age 後半に圧縮: 中盤(0.55)はまだオレンジ＝赤(settled)ではない
  assert.notDeepStrictEqual(agedAutumnColor(0.55, COLOR_PALETTE[2]), COLOR_PALETTE[2], '中盤はまだ赤でない');
  // canopy/particle endpoints が age を尊重し、age=1 で settled(赤)に一致(連続性)
  assert.deepStrictEqual(seasonEndpoints(2, 0).colorCur, AUTUMN_FRESH_COLOR);
  assert.deepStrictEqual(seasonEndpoints(2, 1).colorCur, seasonEndpoints(3, 1).colorPrev, '秋cur(age1)=冬prev');
  assert.deepStrictEqual(particleEndpoints(2, 0).colorCur, AUTUMN_FRESH_COLOR, '落ち葉も age で銀杏→…→赤');
  assert.deepStrictEqual(particleEndpoints(2, 1).colorCur, particleEndpoints(3, 1).colorPrev, '落ち葉 秋cur(age1)=冬prev');
});

test('秋の落ち葉は控えめにハラハラ: 乱気流でなく狭いsway・遅いspin・floaty落下、emit継続', () => {
  const autumn = PARTICLE[2];
  assert.ok(autumn.amount > 0, '最後まで降り続ける(emit > 0)');
  assert.ok(autumn.sway <= 0.25, `狭い横ドリフト(乱気流でない) sway=${autumn.sway}`);
  assert.ok(autumn.spin <= 1.0, `遅いtumble(ゆっくり舞う) spin=${autumn.spin}`);
  assert.ok(autumn.fall < 1.3, `floaty(遅い)落下＝ハラハラ fall=${autumn.fall}`);
});
