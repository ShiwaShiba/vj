...
- 左上の **≡** でコントロールパネルの表示/非表示。手動で閉じるまで開いたままです。
...
本リポジトリのソースコードおよび配信物（ブラウザで表示される JS / CSS / HTML / アセットを含む）は著作権者の専有物です。**無断での複製・改変・再配布・転用を禁止します。** 詳細は [LICENSE](LICENSE) を参照してください。

ブラウザ上での閲覧・個人的な非商用評価は、上記の権利を許諾するものではありません。
